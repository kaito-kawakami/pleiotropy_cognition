#---------------------------------------
# ggmModSelect() algorithm  for GNA
#---------------------------------------

### Import GNA functions to estimate GGM
# Function to obtain sandwich corrected robust standard errors
...
# Function to estimate GGM using LDSC output
...
# simple function to calculate partial correlation matrix from covariance matrix
wi2net <- function(x) {
  x <- -cov2cor(x)
  diag(x) <- 0
  x <- forceSymmetric(x)
...
# function to estimate 100 logarithically spaced lambda values for glasso. If edge = 0, then constrained, otherwise remaining edges freely estimated using ML.  
# GGM with best fit chosen as baseline
...
# used to select the best model using stepwise search
